        ...
        {submissions.map((sub) => (
          <div key={sub.id} className="border rounded-lg p-4">
            <h3 className="font-semibold">
              Request #{sub.id} - {new Date(sub.submitted_at).toLocaleDateString()}
            </h3>
            <p className="text-sm text-neutral-600">{sub.message}</p>
            <p>
              Status: {sub.status || "Pending"} (Updated:{" "}
              {new Date(sub.updated_at || sub.submitted_at).toLocaleDateString()})
            </p>
            {sub.uploaded_files && (
              <div className="mt-2">
